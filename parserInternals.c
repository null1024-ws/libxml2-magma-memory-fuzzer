...
xmlNextChar(xmlParserCtxtPtr ctxt)
{
    ...
    if ((*ctxt->input->cur == 0) &&  // 6 parserInternals.c:436
...
xmlNewParserCtxt(void)
{
	...
	return(NULL);  // 1 parserInternals.c:1867
    }
    memset(ctxt, 0, sizeof(xmlParserCtxt));
    if (xmlInitParserCtxt(ctxt) < 0) {
        xmlFreeParserCtxt(ctxt);
	return(NULL);  // 1 parserInternals.c:1872
    }
    return(ctxt);  // 1 parserInternals.c:1874
}
